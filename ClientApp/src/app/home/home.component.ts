...
export class HomeComponent implements OnInit, OnDestroy {
  ...
  TempConList: TempContractorList[] = [];
    ...
    ProfessionalRegNo = "";
  CIBRating = "";
  CIBDRating = ""; //there's a form with this...
    Name = "";
    Surname = '';
  ...
  dataSourceEngineers = this.EngineersList;
    ...
    openProfSelect(profSelect: any) {
        this.modalService.open(profSelect, { centered: true, size: 'xl' });
    }
  ...
  openOlderProf(oldProf: any) {
    ...
    //NEED TO DISPLAY TABLE AND BE ABLE TO DELETE AND UPDATE RECENTLY ADDED CONTRACTORS
  ...
  saveEditNewTempEngineer() {
    ...
    //NEED TO DISPLAY TABLE AND BE ABLE TO DELETE AND UPDATE RECENTLY ADDED ENGINEERS

    //CREATEPROFESSIONALCREATEPROFESSIONALCREATEPROFESSIONALCREATEPROFESSIONALCREATEPROFESSIONALCREATEPROFESSIONALCREATEPROFESSIONALCREATEPROFESSIONAL
    ...
    ContractorListEU: ProfListEU[] = [];
    EngineerListEU: ProfListEU[] = [];

    getExtUserContractors() {
        this.ContractorListEU.splice(0, this.ContractorListEU.length);

        this.professionalService.getProfessionalsListByProfessionalType(this.userID, "Contractor").subscribe((data: any) => {
            if (data.responseCode == 1) {
                console.log("data.dateSet get", data.dateSet);

                for (let i = 0; i < data.dateSet.length; i++) {
                    const tempContactorList = {} as ProfListEU;
                    const current = data.dateSet[i];
                    ...
                    this.ContractorListEU.push(tempContactorList);
                    //Where am I gonna render this???
                }
            }
  ...
  theirProfessionalID: number | null;
  onAddTheirContractor(showAddedProf: any) {
    /*  professinalID: number;
  ProfessinalType: string;
  professionalRegNo: string;
  bpNumber: string;
  ...
  idNumber?: string;*/
    ...
    /*const newContractor: ContractorList = {
      ...
      idNumber: this.contractorIDNo, //Wait a minute? Yini efunakala la? Company name of i-ID?
      professionalRegNo: this.ProfessionalRegNo,
   ...
   // this.clearCreateComponent();
     ...
     //}
     //debugger;
     // // Push the added contractor into the array
     // //SOMETHING IS OFF!
     // const newContractor: TempContractorList = {

     //   companyName: this.contractorIDNo, //Wait a minute? Yini efunakala la? Company name of i-ID?
     ...
     // };
     //console.log('do you even get here by the temp con list side??', newContractor);
     //debugger;
     // this.TempConList.push(newContractor);
     // debugger;
     // console.log('do you even get here by the temp con list side??');
     // console.log(newContractor);

     // this.TempContractorTable?.renderRows(); // Explicitly trigger rendering

     //   this.professionalService.addUpdateProfessional(0, "Contractor", this.Name + " " + this.Surname,
     //       this.bpNoContractor, false, this.ContractorEmail, this.ContractorTell?.toString(), this.ProfessionalRegNo,
     //       this.userID, this.contractorIDNo, this.CurrentUser.appUserId, this.CIBRating).subscribe((data: any) => {
     //         if (data.responseCode == 1) {
     //           //WHY DOESN'T IT DO ANY OF THESE BELOW??
     //           // Update the professionalID in the new contractor
     //           newContractor.professionalID = data.professionalID;
     //           debugger;
     //           // Access and assign the professionalID from the response data
     //           this.theirProfessionalID = data.professionalID; // Update this based on your API response structure

     //           console.log("This professionalID: " +this.theirProfessionalID);

          
     //           alert(data.responseMessage);
     //           this.getExtUserContractors();
  ...
  onViewAddedContractors(showAddedProf: any) {
    this.openViewTheirProf(showAddedProf);
  }
    getExtUserEngineers() {
        this.EngineerListEU.splice(0, this.EngineerListEU.length);

        this.professionalService.getProfessionalsListByProfessionalType(this.userID, "Engineer").subscribe((data: any) => {
            if (data.responseCode == 1) {
                console.log("data.dateSet get", data.dateSet);

                for (let i = 0; i < data.dateSet.length; i++) {
                    const tempEngineerList = {} as ProfListEU;
                    const current = data.dateSet[i];
                    tempEngineerList.bpNumber = current.bP_Number;
                    tempEngineerList.CIBRating = current.cibRating;
                    ...
                    this.EngineerListEU.push(tempEngineerList);
                    //Where am I gonna render this???
                }
            }
...
//[Next] button
  ...
  // Edit a temporarily saved engineer by index
  editTemporaryEngineer(index: number, updatedEngineer: TempEngineerList) {
    if (index >= 0 && index < this.TempEngList.length) {
      // Check if the index is valid
      this.TempEngList[index] = updatedEngineer;
    } else {
      console.error('Invalid index for editing engineer.');
    }
  }
  ...
  selectedUserName: string = '';
    getUserID(index: any) {
      ...
      //How can I get this user's contractor's and engineers?
    ...
    modalEngineer: boolean;
    modalContractor: boolean;

    populateClientInfo() {
        console.log("I get here with Turtle Speed on my mind still...");
        debugger;
        if (!this.modalEngineer && !this.modalContractor) {
    ...
    createWayleave(applicationType: boolean, isPlanning: boolean) {
      ...
      if (this.option == "client") {
        debugger;
        this.userID = this.sharedService.userIDForWalkIn;
       
            this.NewWayleaveComponent.onWayleaveCreate(this.userID, isPlanning);
            console.log("Who becomes turtle? You: " + this.userID); //Nope, this is the same ID I selected
        ...
        debugger;
            ...
            console.log("...or you: " + this.CurrentUser.appUserId);
      }
      debugger;
    ...
    onFilterApplications() {
        if (this.select == "option1") {
            ...
            this.myApplicaitonsUnpaidCount();
            this.myApplicaitonsWIPCount();
            this.myApplicaitonsejectedCount();
            this.myApplicaitonsApprovedCount();
            this.myApplicaitonsDistributedCount();

            /*      this.filterByUnpaid();
                  this.filterByDistribution();
                  this.filterByApproved();
                  this.filterByRejected();
                  this.filterByWIP();*/
            /* this.Applications = [];
