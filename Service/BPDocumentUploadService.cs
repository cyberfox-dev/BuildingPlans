...
{
    public class BPDocumentUploadService : IBPDocumentUploadService
    {

        private readonly AppDBContext _context;

        public BPDocumentUploadService(AppDBContext context)
        {
            _context = context;
        }
        ...
        public async Task<BPDocumentUploads> AddUpdateBPDocument(int? documentID, string documentName, string? DocumentLocalPath, int? applicationID, string? assignedUserID, string createdById, string? groupName, int? subDepID, string? subDepName, bool? isPlanning, bool? isRepository, string? description)
        {
            ...
            //this checks is the record exists in the db
            var tempDocumentUpload = _context.BPDocumentUploads.FirstOrDefault(x => x.DocumentID == documentID);

            //if the object is null assume that the user is tying to add a new Document
            if (tempDocumentUpload == null)
            {
                //create a new object of DocumentUpload entity class then initialize the object with given infomation
                tempDocumentUpload = new BPDocumentUploads()
                {
                    DocumentName = documentName,
                    DocumentLocalPath = DocumentLocalPath,
                    ApplicationID = applicationID,
                ...
                //After the inizlization add to the db
                await _context.BPDocumentUploads.AddAsync(tempDocumentUpload);
                await _context.SaveChangesAsync();
        ...
        public async Task<bool> DeleteBPDocument(int documentID)
        {
            var tempDocumentUpload = _context.BPDocumentUploads.FirstOrDefault(x => x.DocumentID == documentID);
        ...
        public async Task<List<DocumentUploadDTO>> GetAllBPDocumentsForRepository()
        {


            return await (
              from documentUpload in _context.BPDocumentUploads
              where documentUpload.isRepository == true && documentUpload.isActive == true
              select new DocumentUploadDTO()
              {
                  DocumentID = documentUpload.DocumentID,
        ...
        public async Task<List<DocumentUploadDTO>> GetAllBPDocumentsForApplication(int? applicationID)
        {
