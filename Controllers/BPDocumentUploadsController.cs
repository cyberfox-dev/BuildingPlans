...
namespace BuildingPlans.Controllers
{
    ...
    [ApiController]
    public class BPDocumentUploadsController : ControllerBase
    {

        private readonly string ApplicationDirectory = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot");
        private static List<BPDocumentUploads> fileForDb = new List<BPDocumentUploads>();
        ...
        public async Task<object> AddUpdateDocument([FromBody] DocumentUploadBindingModel model)
        {
            try
            {
                ...
                {
                    var result = await _documentUploadService.AddUpdateBPDocument(model.DocumentID, model.DocumentName, model.DocumentLocalPath, model.ApplicationID, model.AssignedUserID, model.CreatedById, model.DocumentGroupName, model.SubDepartmentID, model.SubDepartmentName, model.isPlanning, model.isRepository, model.DocumentLocalPath);
        ...
        public async Task<object> DeleteDocument([FromBody] int documentID)
        {
            try
            {
                ...
                {
                    var result = await _documentUploadService.DeleteBPDocument(documentID);
        ...
        [HttpGet("GetAllBPDocumentsForRepository")]
        public async Task<object> GetAllBPDocumentsForRepository()
        {
            try
            {

                var result = await _documentUploadService.GetAllBPDocumentsForRepository();
